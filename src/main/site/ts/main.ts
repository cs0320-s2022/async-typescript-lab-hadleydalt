...
const suggestionList = document.querySelector('#suggestions') as HTMLElement
const dropDownSun = document.querySelector('#sun') as HTMLOptionElement
const dropDownMoon = document.querySelector('#moon') as HTMLOptionElement
const dropDownRising = document.querySelector('#rising') as HTMLOptionElement

// Here, when the value of sun is changed, we will call the method postAndUpdate.
// TODO: Do the same for moon and rising

dropDownSun.addEventListener('change', postAndUpdate);
dropDownMoon.addEventListener('change', postAndUpdate);
dropDownRising.addEventListener('change', postAndUpdate);
...
function postAndUpdate(): void {
  ...
  const postParameters: MatchesRequestData = {
    sun: dropDownSun.value,
    moon: dropDownMoon.value,
    rising: dropDownRising.value
  ...
  fetch('http://localhost:4567/results', {
    method: 'post',
    body: JSON.stringify({
      sun: postParameters.sun,
      moon: postParameters.moon,
      rising: postParameters.rising
    }),
    headers: {
...
function updateSuggestions(matches: string[]): void {
  ...
  matches.map((match) => suggestionList.innerHTML += `<li tabindex="0">${match}</li>`)
}
...
document.addEventListener("keyup", async()=>{
  await updateValues("Scorpio", "Gemini", "Scorpio")
  postAndUpdate()
})
...
async function updateValues(sunval: string, moonval: string, risingval: string): Promise<void>{
  ...
  dropDownSun.value = sunval;
  dropDownMoon.value = moonval;
  dropDownRising.value = risingval;
}
