...
public final class Main {
  ...
  private static class ResultsHandler implements Route {
    ...
    public String handle(Request req, Response res) throws JSONException {
      ...
      assert reqJson != null;
      sunVal = reqJson.getString("sun");
      moonVal = reqJson.getString("moon");
      risingVal = reqJson.getString("rising");

      // TODO: use the MatchMaker.makeMatches method to get matches

      List<String> horoscopeVals = MatchMaker.makeMatches(sunVal, moonVal, risingVal);

      // TODO: create an immutable map using the matches

      Gson GSON = new Gson();
      Map<String, Object> horoscopes = ImmutableMap.of("matches", horoscopeVals);

      // TODO: return a json of the suggestions (HINT: use GSON.toJson())

      return GSON.toJson(horoscopes);
    }
  }
}
